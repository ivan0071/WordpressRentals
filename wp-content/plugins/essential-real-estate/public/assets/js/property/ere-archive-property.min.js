(function ($) {
    'use strict';
    $(document).ready(function () {
        function ere_archive_property() {
            $('.property-filter-placeholder', '.archive-property-action .sort-property').on('click', function(event) {
                var $this = $(this);
                event.preventDefault();
                if(!$this.next('ul').hasClass('active')) {
                    $this.next('ul').addClass('active');
                    $this.addClass('active');
                } else {
                    $this.next('ul').removeClass('active');
                    $this.removeClass('active');
                }
                return false;
            });
            $('li', '.archive-property-action .sort-property').each(function() {
                var $this = $(this);
                if(window.location.href.indexOf("sortby="+$this.children().data('sortby')) > -1) {
                    $this.parent().children('li').removeClass('active');
                    $this.addClass('active');
                    $this.closest('ul').prev('span').html($this.children().html());
                }
                $this.on('click', 'a', function(event){
                    $(this).closest('ul').removeClass('active');
                    if($(this).parent().hasClass('active')) {
                        event.preventDefault();
                        return false;
                    } else {
                        $(this).closest('ul').prev('span').html($(this).html());
                    }
                });
            });
            $('li', '.archive-property-action .property-status').each(function() {
                var $this = $(this);
                if(window.location.href.indexOf("status="+$this.children().data('status')) > -1) {
                    $this.parent().children('li').removeClass('active');
                    $this.addClass('active');
                    $this.closest('ul').prev('span').html($this.children().html());
                }
            });
            $(document).on('click', function (e) {
                if ($(e.target).closest('.sort-property').length == 0) {
                    $('ul', '.sort-property').each(function () {
                        var $this = $(this);
                        if ($this.hasClass('active')) {
                            $this.removeClass('active');
                            $this.prev('span').removeClass('active');
                        }
                    });
                }
            });
            $('span', '.archive-property-action .view-as').each(function() {
                var $this = $(this);
                if(window.location.href.indexOf("view_as") > -1 ){
                    if(window.location.href.indexOf("view_as="+$this.data('view-as')) > -1) {
                        $this.addClass('active');
                        manage_layout_content($(this), $(this).data('view-as'));
                    }
                } else {
                    if($('.ere-property', '.ere-property-wrap').hasClass($this.data('view-as'))) {
                        $this.addClass('active');
                        manage_layout_content($(this), $(this).data('view-as'));
                    }
                }
                var handle = true;
                $this.on('click', function(event){
                    var $view = $(this),
                        $view_as = $view.data('view-as'),
                        $property_list = $view.closest('.ere-property-wrap').find('.ere-property'),
                        $property_container_no_map = $view.closest('.ere-property-wrap').find('.map-no'),
                        $property_container_yes_map = $view.closest('.ere-property-wrap').find('.map-yes'),
                        $ajax_url = $view.closest('.view-as').data('admin-url');
                    event.preventDefault();
                    manage_layout_content($(this), $view_as);

                    if($view.hasClass('active') || !handle) {
                        return false;
                    } else {
                        handle = false;
                        $view.closest('.view-as').find('span').removeClass('active');
                        $view.addClass('active');
                        $property_list.fadeOut();
                        setTimeout(function () {
                            if($view_as=='property-list') {
                                $property_list.removeClass('property-grid').addClass('property-list list-1-column');
                                //$property_list.removeClass('property-grid').removeClass('property-map').addClass('property-list list-1-column');
                                $property_container_no_map.removeClass("hiddenClass");
                                $property_container_yes_map.addClass("hiddenClass");
                            } else if($view_as=='property-grid') {
                                $property_list.removeClass('property-list list-1-column').addClass('property-grid');
                                //$property_list.removeClass('property-list list-1-column').removeClass('property-map').addClass('property-grid');
                                $property_container_no_map.removeClass("hiddenClass");
                                $property_container_yes_map.addClass("hiddenClass");
                            } else if($view_as=='property-map') {
                                //$property_list.removeClass('property-list list-1-column').removeClass('property-grid').addClass('property-map');
                                $property_container_no_map.addClass("hiddenClass");
                                $property_container_yes_map.removeClass("hiddenClass");
                            }
                            $property_list.fadeIn('slow');
                        }, 400);
                        $.ajax({
                            url: $ajax_url,
                            data: {
                                action: 'ere_property_set_session_view_as_ajax',
                                view_as: $view_as
                            },
                            success: function () {
                                handle = true;
                            },
                            error: function () {
                                handle = true;
                            }
                        });
                    }
                });
            });
        }
        ere_archive_property();

        function manage_layout_content(thisObj, view_as) {
            var property_container_grid = thisObj.closest('.ere-property-wrap').find('.property-item-content-grid');
            var property_container_list = thisObj.closest('.ere-property-wrap').find('.property-item-content-list');

            if(view_as=='property-grid') {
                property_container_grid.removeClass("hiddenClass");
                property_container_list.addClass("hiddenClass");
            } else if(view_as=='property-list') {
                property_container_list.removeClass("hiddenClass");
                property_container_grid.addClass("hiddenClass");
            } else {
                property_container_list.addClass("hiddenClass");
                property_container_grid.addClass("hiddenClass");
            }
        }

        function ere_property_paging_control() {
            $('.paging-navigation', '.ere-property').each(function () {
                var $this = $(this);
                if($this.find('a.next').length === 0) {
                    $this.addClass('next-disable');
                } else {
                    $this.removeClass('next-disable');
                }
            });
        }
        ere_property_paging_control();
       
        /*function ere_property_search_paging_control() {
            $('.paging-navigation a', '#home-search-wrap').click(function () {
                var $this = $(this);
                var clickedPage = $this.text();
                var currentPage = 1;
                var regex = /[?&]([^=#]+)=([^&#]*)/g,
                    url = window.location.href,
                    params = {},
                    match;
                while(match = regex.exec(url)) {
                    params[match[1]] = match[2];
                }
                if (params != null && params !== undefined && 
                    params.paged != null && params.paged !== undefined)
                {
                    currentPage = parseInt(params.paged);
                }

                if (clickedPage == "Previous") {
                    if (currentPage > 1) {
                        clickedPage = currentPage - 1;
                    } else {
                        clickedPage = 1;
                    }
                } else if (clickedPage == "Next") {
                    if (currentPage > 0) {
                        clickedPage = currentPage + 1;
                    } else {
                        clickedPage = 1;
                    }
                } else {
                    clickedPage = parseInt(clickedPage);
                }

                window.location.href = window.location.href += "?paged=" + clickedPage;
            });
        }
        ere_property_search_paging_control();*/

        $("#ere_save_search").click(function(e) {
            e.preventDefault();
            var $this = $(this);
            var $form = $('#ere_save_search_form');
            var $ajax_url=$this.data( 'ajax-url');
            $.ajax({
                url: $ajax_url,
                data: $form.serialize(),
                method: $form.attr('method'),
                dataType: 'JSON',
                beforeSend: function () {
                    $this.children('i').remove();
                    $this.append('<i class="fa fa-spinner fa-spin"></i>');
                },
                success: function (response) {
                    if (typeof(response.success) == 'undefined') {
                        ERE.login_modal();
                    }
                    if (response.success) {
                        $this.children('i').removeClass('fa-spinner fa-spin');
                        $this.children('i').addClass('fa-check');
                    }
                },
                error: function () {
                    $this.children('i').removeClass('fa-spinner fa-spin');
                    $this.children('i').addClass('fa-exclamation-triangle');
                },
                complete: function () {
                    $this.children('i').removeClass('fa-spinner fa-spin');
                }
            });
        });
    });
})(jQuery);